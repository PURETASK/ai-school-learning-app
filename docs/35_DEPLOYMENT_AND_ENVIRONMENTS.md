...
Define how the app moves from local development to preview and production.
...
- Supabase for database/auth if selected.
...
- Production deploy only after basic QA.
...
- Env vars configured.
- Seed data loaded.
- Student demo flow works.
- Parent demo flow works.

## Implementation Notes for Codex

- Create `.env.example`.
- Never create real secrets in repo.
...
- Define local/dev/preview/production environments, env vars, deployment checks, and rollback plan.
