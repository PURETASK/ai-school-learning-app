# Feedback Engine — V2 Design + Build Specification

## 1. Purpose

Convert quiz evidence into supportive, actionable next steps.
...
- all items wrong
- all items strong
- open responses need review
- missing reteach path

## 9. Acceptance Criteria

- [ ] Uses mastery band to choose next action
- [ ] Includes supportive non-shaming language
- [ ] Names weak skill signals
- [ ] Provides parent support note
- [ ] Links to reteach/challenge logic
...
| Event | Trigger |
|---|---|
| `feedback-engine_viewed` | User lands on the module. |
| `feedback-engine_action_started` | User begins main action. |
| `feedback-engine_action_completed` | User completes main action. |
| `feedback-engine_error_seen` | User sees recoverable error state. |
...
Feedback must map to mastery band, weak skill signals, misconception targets, and next action. Generic “wrong” messages are forbidden.
