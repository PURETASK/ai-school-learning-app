# Lesson Player — V2 Design + Build Specification

## 1. Purpose

Run the universal lesson flow and make every lesson section visible, navigable, and reviewable.
...
- metadata header
- objective/essential question panel
- section navigation rail
- progress meter
- recursive content renderer
- completion cue
- keyboard navigable section jump
- quiz transition
...
- no quiz after lesson
- resume position not found

## 9. Acceptance Criteria

- [ ] Renders all 13 universal sections
- [ ] Supports back/next/jump
- [ ] Marks section completion
- [ ] Never crashes on nested lesson objects
- [ ] Routes to quiz only from lesson flow
...
| Event | Trigger |
|---|---|
| `lesson-player_viewed` | User lands on the module. |
| `lesson-player_action_started` | User begins main action. |
| `lesson-player_action_completed` | User completes main action. |
| `lesson-player_error_seen` | User sees recoverable error state. |
...
Must render all 16 official sections. Sections 13, 14, and 16 may be derived from mastery, Memory Vault, reteach, and challenge metadata.
