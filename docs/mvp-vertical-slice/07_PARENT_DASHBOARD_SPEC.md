# Parent Dashboard — V2 Design + Build Specification

## 1. Purpose

Show a safe progress summary and specific support recommendations.
...
- many weak skills
- parent not linked later

## 9. Acceptance Criteria

- [ ] Shows only linked student data
- [ ] Explains latest mastery and next action
- [ ] Shows weak skill signals
- [ ] Shows Memory Vault items by subject
...
| Event | Trigger |
|---|---|
| `parent-dashboard_viewed` | User lands on the module. |
| `parent-dashboard_action_started` | User begins main action. |
| `parent-dashboard_action_completed` | User completes main action. |
| `parent-dashboard_error_seen` | User sees recoverable error state. |
...
Must show only linked-student data, summarize progress in parent-safe language, and avoid exposing other students or raw sensitive free text.
