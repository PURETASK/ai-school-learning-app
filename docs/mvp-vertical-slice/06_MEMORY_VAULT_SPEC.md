# Memory Vault — V2 Design + Build Specification

## 1. Purpose

Schedule spaced retrieval reviews from completed lessons.
...
- subject grouping
- upcoming/due helpers

## 5. Required Data Inputs

- Lesson.memoryVaultItems
- studentId
- baseDate
- DEFAULT_MEMORY_VAULT_DAYS
- ScheduledReviewItem[]
...
- [ ] Schedules Day 1/3/7/14/30 by default
- [ ] Creates stable IDs
- [ ] Stores dueDateIso
- [ ] Groups by subject
- [ ] Does not mark lesson complete without review scheduling
...
| Event | Trigger |
|---|---|
| `memory-vault_viewed` | User lands on the module. |
| `memory-vault_action_started` | User begins main action. |
| `memory-vault_action_completed` | User completes main action. |
| `memory-vault_error_seen` | User sees recoverable error state. |
...
Must schedule Day 1/3/7/14/30 items and later close the loop with review completion, confidence, adaptive rescheduling, and parent visibility.
...
# V4 Review-Session Addendum

The Memory Vault must include a live review-session workflow, not only scheduled review previews.

## Required V4 Behaviors

- Select due review items first.
- Permit upcoming-item practice in MVP/demo mode if no items are due.
- Require student answer before expected answer reveal.
- Capture confidence rating.
- Score answer using `scoreMemoryVaultResponse`.
- Apply `gradeMemoryVaultAnswer` to create an outcome.
- Apply `applyMemoryVaultReviewSession` to update the full Memory Vault item list.
- Mark strong/correct items completed.
- Reschedule weak/incorrect items for near-term review.
- Store session summaries in the MVP local vertical-slice state.
- Send latest session summary to the Parent Dashboard.
...
Parent Dashboard must show latest Memory Vault session count, average score, correct count, and reteach/rescheduled count.
