# Quiz Engine — V2 Design + Build Specification

## 1. Purpose

Collect evidence of understanding through objective and open-response items.
...
```txt
src/features/quizzes/components/QuizPanel.tsx + src/features/quizzes/quizEngine.ts
```
...
- [ ] Requires every item answered before submit
- [ ] Scores multiple choice exactly
- [ ] Scores open responses heuristically for MVP
- [ ] Produces per-question feedback
- [ ] Produces skill breakdown
...
| Event | Trigger |
|---|---|
| `quiz-engine_viewed` | User lands on the module. |
| `quiz-engine_action_started` | User begins main action. |
| `quiz-engine_action_completed` | User completes main action. |
| `quiz-engine_error_seen` | User sees recoverable error state. |
...
Multiple choice can be auto-scored. Open response, CER, discussion, and interpretation items require rubric review or explicit heuristic-disclaimer state.
