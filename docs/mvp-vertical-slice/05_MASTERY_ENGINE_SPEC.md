# Mastery Engine — V2 Design + Build Specification

## 1. Purpose

Calculate mastery bands and power adaptive branching.
...
- no quiz results
- partial open responses
- missing subject data

## 9. Acceptance Criteria

- [ ] Uses official bands 0-39/40-64/65-79/80-89/90-100
- [ ] Returns one clear next action
- [ ] Supports subject-level summaries
- [ ] Does not conflate completion with mastery
...
| Event | Trigger |
|---|---|
| `mastery-engine_viewed` | User lands on the module. |
| `mastery-engine_action_started` | User begins main action. |
| `mastery-engine_action_completed` | User completes main action. |
| `mastery-engine_error_seen` | User sees recoverable error state. |
...
Must use official five-band model. Mastery is evidence-based, not lesson-completion-based.
