...
Define the security model for accounts, roles, student data, and protected routes.

## Security Philosophy

Assume student data is sensitive. Restrict access by default.
...
- Teacher-class enrollment checks
...
- Teachers access only assigned classes/students.
- School admins access only their school.
- Platform admins have controlled elevated access.

## Data Protection Rules

- Do not log sensitive student data.
- Do not expose raw database errors.
- Validate all user-submitted fields.
- Use server-side checks, not only UI hiding.

## Implementation Notes for Codex

- Create central permission helpers.
- Do not duplicate role logic across components.
- Add tests for permission boundaries later.
...
- Define auth, authorization, data protection, secure logs, protected routes, and secrets handling.
