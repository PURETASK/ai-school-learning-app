...
Phase completion evidence is included in each learner-scoped catalog lesson as `phaseCompletions` and `completedPhaseCount`. The lesson player uses this repository-backed evidence after a scoped refresh, while its local event state remains a compatibility fallback for preview mode.
...
- `src/repository.js` provides JSON fallback, Postgres and Supabase PostgREST snapshot persistence, normalized table upsert SQL/REST writes, targeted account provisioning, focused write slices for account security, learning evidence, content workflow, visual workflow, and manager review decisions, the normalized account-security read model, the normalized learning-catalog read model, learner-scoped reward/tutor/portfolio/badge evidence read models, staff-scoped classroom evidence read model, school-operations read model, the normalized content-draft read model, the normalized visual-asset read model, the manager review queue read model, and the audit-event read model.
