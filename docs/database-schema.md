...
`createNormalizedTableDeleteMissingSql(state, ["agent_review_items"])` is used only for the derived manager review queue so stale pending review rows are removed after an approval, rejection, or reviewed decision. It is intentionally not used on learner evidence or content history tables.
