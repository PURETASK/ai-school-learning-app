# Sample Content Seed Plan

## Purpose

Define the first sample lessons and curriculum files used to test the app.

## MVP Seed Content Goal

Create enough content to test the full learning loop without building full grade curricula.
...
- 1 complete lesson per academy
- 1 quiz per lesson
- 1 review item per lesson
- 1 mastery record seed
- 1 sample student profile
- 1 sample parent profile

## Implementation Notes for Codex

- Keep seed data in `/content` or `/src/data/seed`, not inside UI components.
...
- Seed one high-quality vertical slice per MVP grade/subject before scaling.
