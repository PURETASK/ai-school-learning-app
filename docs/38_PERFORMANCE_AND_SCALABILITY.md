...
Define performance rules so the app stays fast as curriculum and users grow.

## MVP Performance Goals

- Dashboard loads quickly.
- Lesson player transitions smoothly.
- Quiz scoring is instant.
- Review due items are easy to query.

## Risks

- Too much curriculum loaded client-side.
- Large dashboards with unoptimized queries.
- Heavy animations hurting low-end devices.
- Overcomplicated state management.

## Rules

- Load only needed lesson content.
- Paginate long lists later.
- Cache static curriculum where appropriate.
- Avoid heavy client bundles.
- Keep animations lightweight.

## Later Scalability Concerns

- Schools/classes at scale.
- Large question banks.
- Reporting aggregation.
- AI tutor cost and latency.

## Implementation Notes for Codex

- Do not import the entire curriculum into every page.
...
- Define performance budgets for lesson load, dashboard load, content parsing, and media usage.
