...
Define how database schema and sample data will be introduced when the project moves from content files to database-backed data.

## MVP Position

Start with Markdown/JSON content files first. Add database persistence after the lesson format stabilizes.
...
- Sample Memory Vault items

## Future Tables

Reference `docs/16_DATA_MODEL_SPEC.md` for canonical table list.

## Implementation Notes for Codex

- Create seed scripts only after stack is chosen.
- Keep seed data separated from production logic.
- Use anonymized demo names.
...
- Define migrations, seed data, local dev reset, and content import process.
