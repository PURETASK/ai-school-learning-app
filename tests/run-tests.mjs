...
const afterProofProgress = getNexusPhaseProgress(completedProof.state, phaseGateLearnerId, phaseGateLessonId);
assert.equal(afterProofProgress.nextPhase, null, "After prove, delayed recall should wait for a written retrieval response");
assert.equal(afterProofProgress.phases.find((phase) => phase.phase === "remember")?.reason, "Write what you can recall without looking before clearing this phase.", "Remember should explain its evidence requirement");
const missingRecallEvidence = completeNexusPhase(completedProof.state, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, phase: "remember" });
assert.equal(missingRecallEvidence.result.accepted, false, "Remember should require a delayed-recall response");
phaseGateState = updateLessonScratchpad(completedProof.state, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, recallResponse: "A unit rate compares one unit so I can compare fairly later." });
const completedRemember = completeNexusPhase(phaseGateState, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, phase: "remember" });
assert.equal(completedRemember.result.accepted, true, "A saved recall response should unlock the remember phase");
const missingTransferEvidence = completeNexusPhase(completedRemember.state, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, phase: "transfer" });
assert.equal(missingTransferEvidence.result.accepted, false, "Transfer should require a new-situation explanation");
phaseGateState = updateLessonScratchpad(completedRemember.state, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, transferResponse: "I can use the same comparison for a recipe by finding the amount per serving." });
const completedTransfer = completeNexusPhase(phaseGateState, { learnerId: phaseGateLearnerId, lessonId: phaseGateLessonId, phase: "transfer" });
assert.equal(completedTransfer.result.accepted, true, "A saved transfer response should unlock the transfer phase");
